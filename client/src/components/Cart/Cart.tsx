...
const Cart =()=>{
    ...
    const getcart = async():Promise<void>=>{
        try{
            let response = await axios.get(`https://e-commerce-wvh2.vercel.app/api/v1/cart/getCart`,{withCredentials:true})
            if(response.data){
    ...
    const removeItem = async(id:string):Promise<void>=>{
        try{
            setloader(true)
            let response = await axios.post(`https://e-commerce-wvh2.vercel.app/api/v1/cart/removeItem`,{itemId:id,cartId:cart?.id},{withCredentials:true})
            if(response.data){
                setloader(false)
                setCart(response.data.cart)
            }
    ...
    const checkout = async():Promise<void>=>{
        try{
            setloader(true)
            let response = await axios.post(`https://e-commerce-wvh2.vercel.app/api/v1/checkout`,{item:cart},{withCredentials:true})
    ...
    const addItem = async(id:string,price:number):Promise<void>=>{
        try{
            setloader(true)
            let response = await axios.post(`https://e-commerce-wvh2.vercel.app/api/v1/cart/addItem`,{item:id,pricetopay:price},{withCredentials:true})
            setCart(response.data.cart)
            setloader(false)
        }catch(err){
            setloader(false)
