...
const AllProduct =()=>{
    ...
    const getProducts = async():Promise<void>=>{
        let query:string = `https://e-commerce-wvh2.vercel.app/api/v1/item/getAllitems?page=${page}&limit=20&${converttoString(querybrands,"manufacturour")}&${converttoString(querycategories,"category")}&price[gte]=${lprice}&price[lte]=${gprice}&sort=${sort}&fields=_id,price,title,coverphoto,discount`
        try{
            setloader(true)
    ...
    const getBrandAndCate =async():Promise<void>=>{
        try{
            setloader(true)
            let brandsres = await axios.get('https://e-commerce-wvh2.vercel.app/api/v1/brands/getAllbrands',{withCredentials:true})
            if(brandsres.data){
                setbrands(brandsres.data.brands);
            }
            let categoryres = await axios.get('https://e-commerce-wvh2.vercel.app/api/v1/category',{withCredentials:true})
            if(categoryres.data){
