...
const updateData =async(data:FieldValues):Promise<void>=>{
    try{
        setloader(true)
        let resposne = await axios.patch(`https://e-commerce-wvh2.vercel.app/api/v1/item/update/${item?._id}`,data,{withCredentials:true});
        if(resposne.data){
...
const deletephoto =async(id:string):Promise<void>=>{
    try{
        setloader(true)
        let response = await axios.get(`https://e-commerce-wvh2.vercel.app/api/v1/item/removephoto/${item?._id}?photoid=${id}`,{withCredentials:true})
        if(response.data){
            setloader(false)
...
const updateCoverphoto =async():Promise<void>=>{
    if(photo){
        try{
            setloader(true)
            let response = await axios.patch(`https://e-commerce-wvh2.vercel.app/api/v1/item/updateCoverphoto/${item?._id}`,{profile:photo},{withCredentials:true,headers:{'Content-Type':'multipart/form-data'}})
            if(response.data){
                setitem(response.data.item)
                setphoto(null);
                setloader(false)
...
const updatephotos =async():Promise<void>=>{
    if((photos? photos.length:0)>0){
        ...
        try{
            setloader(true)
            let response = await axios.patch(`https://e-commerce-wvh2.vercel.app/api/v1/item/uploadphoto/${item?._id}`,formData,{withCredentials:true,headers:{
  ...
  const getItem =async():Promise<void>=>{
    try{
        setloader(true);
        let response = await axios.get(`https://e-commerce-wvh2.vercel.app/api/v1/item/getItem/${id}`,{withCredentials:true})
