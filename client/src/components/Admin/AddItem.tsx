...
const AddItem = () => {
    ...
    const uploadPhoto = async():Promise<{url:string,id:string} | null | undefined>=>{
        if(photo){
            try{
                let response = await axios.post('http://13.211.135.249:8000/api/v1/item/uploadCoverphoto',{profile:photo},{
        ...
        console.log(data)
            ...
            try{
                if(photodata){
                    ...
                    try{
                        let response = await axios.post(`http://13.211.135.249:8000/api/v1/item/`,data,{withCredentials:true})
                        if(response.data){
                            setloader(false)
