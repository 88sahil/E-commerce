...
const AddItem = () => {
    ...
    const uploadPhoto = async():Promise<{url:string,id:string} | null | undefined>=>{
        if(photo){
            try{
                let response = await axios.post('https://mpfserver.vercel.app/api/v1/item/uploadCoverphoto',{profile:photo},{
        ...
        console.log(data)
            ...
            try{
                if(photodata){
                    ...
                    try{
                        let response = await axios.post(`https://mpfserver.vercel.app/api/v1/item/`,data,{withCredentials:true})
                        if(response.data){
                            setloader(false)
