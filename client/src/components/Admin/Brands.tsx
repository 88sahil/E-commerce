...
const Brands = () => {
    ...
    const UploadLogo =async():Promise<{url:string,id:string} | null | undefined>=>{
        if(photo){
            try{
                let response = await axios.post(`https://mpfserver.vercel.app/api/v1/brands/uploadphoto`,{profile:photo},{withCredentials:true,headers:{
    ...
    const getBrands = async():Promise<void>=>{
                setloader(true)
                try{
                let response = await axios.get(`https://mpfserver.vercel.app/api/v1/brands/getAllbrands`,{withCredentials:true})
            if(response.data){
                setloader(false)
    ...
    const deleteBrand = async(id:string):Promise<void>=>{
        try{
            setloader(true)
            let response = await axios.delete(`https://mpfserver.vercel.app/api/v1/brands/deletebrand/${id}`,{withCredentials:true})
            if(response.data){
                setloader(false)
    ...
    const AddBrand =async(data:FieldValues):Promise<void>=>{
        if(photo){
            ...
            try{
                let response = await axios.post(`https://mpfserver.vercel.app/api/v1/brands/AddBrand`,data,{withCredentials:true})
                if(response.data){
                    setloader(false)
