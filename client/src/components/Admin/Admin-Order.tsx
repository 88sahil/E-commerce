...
const AdminOrder = () => { 
    ...
    let GetOrders=async():Promise<void>=>{
        try{
            let response = await axios.get(`https://e-commerce-wvh2-qw50sstcd-88sahils-projects.vercel.app/api/v1/Orders/getAllOrder?type=${status}`)
            if(response.data){
    ...
    <main className='w-full flex max-md:justify-start justify-center'>
        <div className='w-[900px] max-md:w-full p-2'>
            <div className='flex justify-between items-center'>
                <h1 className='text-xl font-bold'>OrderStatus:<span className={`${status==="delivered"? "text-green-600":(status==="rejected"? "text-red-600":"text-blue-700")}`}>{status || "All"}</span></h1>
                <h1 className='text-xl font-bold'>Results:{results}</h1>
