...
const Category = () => {
    ...
    const getCategory =async():Promise<void>=>{
        try{
            setloader(true)
            let response = await axios.get(`http://13.211.135.249:8000/api/v1/category`);
            if(response.data){
    ...
    const addCategory =async(data:FieldValues):Promise<void>=>{
        try{
            setloader(true)
            let response = await axios.post(`http://13.211.135.249:8000/api/v1/category`,data,{withCredentials:true})
            if(response.data){
                setloader(false)
