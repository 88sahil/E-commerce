...
const ItemCard = (props:{item:Item,key:number})=>{
    ...
    const LikeItem =async(id:string)=>{
        try{
            let response = await axios.get(`https://mpfserver.vercel.app/api/v1/user/LikeItem?Itemid=${id}`,{withCredentials:true})
            if(response.data){
    ...
    return(
        <NavLink to={`/product/${props.item._id}`} key={props.key} className="w-[250px] p-2 max-md:h-[300px] max-md:w-[180px] mt-3 h-[350px] shadow-md">
            <img src={props.item.coverphoto} alt="coverphoto" className="w-full max-md:h-[200px] h-[220px]  object-contain"/>
            <h3 className=" px-2 text-xl w-full mt-[5px]  text-center"><BiLike size={25} onClick={()=>handleLikebtn()} color={`${liked?"red":""}`}/></h3>
            <p className="px-2 text-md w-full overflow-hidden whitespace-nowrap max-md:text-sm font-bold text-start">{props.item.title}</p>
