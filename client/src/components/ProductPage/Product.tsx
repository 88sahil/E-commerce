...
const Product =()=>{
    ...
    const addtocart = async()=>{
        try{
            setloader(true)
            let response = await axios.post("https://e-commerce-wvh2-qw50sstcd-88sahils-projects.vercel.app/api/v1/cart/addItem",{item:item?._id,pricetopay:item?.price? item.price:0-(item?.discount? item?.discount:0)},{withCredentials:true})
            if(response.data){
                setloader(false)
            }
        }catch(err){
    ...
    const postrating =async(data:FieldValues):Promise<void>=>{
        try{
            setloader(true)
            let response = await axios.post(`https://e-commerce-wvh2-qw50sstcd-88sahils-projects.vercel.app/api/v1/review/postreview/${item?._id}`,data,{withCredentials:true})
            if(response.data){
    ...
    const deleteReview = async(id:string):Promise<void> =>{
        try{
            setloader(true)
            let response = await axios.get(`https://e-commerce-wvh2-qw50sstcd-88sahils-projects.vercel.app/api/v1/review/deleteReview/${id}`,{withCredentials:true})
            if(response.data){
