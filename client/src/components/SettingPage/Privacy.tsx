...
const Privacy=()=>{
    ...
    const changepassword =async(data:FieldValues):Promise<void>=>{
        try{
            setloader(true);
            let response = await axios.patch("https://mpfserver.vercel.app/api/v1/user/changepassword",data,{withCredentials:true})
            if(response.data){
                setloader(false)
    ...
    const deleteUser = async ():Promise<void>=>{
        setloader(true)
        try{
            let response = await axios.patch('https://mpfserver.vercel.app/api/v1/user/updateMe',{isactive:false},{withCredentials:true})
            if(response.data){
                setloader(false)
                dispatch(logout())
                navigate('/')
