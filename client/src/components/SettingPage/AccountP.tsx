...
const AccountPre=()=>{
    ...
    const UploadProfile =async()=>{
        try{
            if(img){
                setloader(true)
                let response = await axios.patch('http://13.211.135.249:8000/api/v1/user/uploadprofile',{profile:img},{
    ...
    const updateUserData=async(data:FieldValues):Promise<void>=>{
        ...
        try{
            setisDisable(true)
            setloader(true)
            let response = await axios.patch("http://13.211.135.249:8000/api/v1/user/updateMe",data,{withCredentials:true})
            if(response.data){
                setloader(false)
                dispatch(login(response.data.user))
            }
    ...
    const updatelocation =async(data:FieldValues):Promise<void>=>{
        ...
        try{
            setloader(true)
            let response = await axios.patch("http://13.211.135.249:8000/api/v1/user/updateMe",{location:AddressObj},{withCredentials:true})
            if(response.data){
                setloader(false)
                dispatch(login(response.data.user))
            }
