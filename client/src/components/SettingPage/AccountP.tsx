...
const AccountPre=()=>{
    ...
    const updateUserData=async(data:Tdata):Promise<void>=>{
        ...
        try{
            setisDisable(true)
            setloader(true)
            let response = await axios.patch("/api/v1/user/updateMe",data,{withCredentials:true})
            if(response.data){
                setloader(false)
                dispatch(login(response.data.user))
            }
    ...
    const updatelocation =async(data:Tlocation):Promise<void>=>{
        ...
        try{
            setloader(true)
            let response = await axios.patch("/api/v1/user/updateMe",{location:AddressObj},{withCredentials:true})
            if(response.data){
                setloader(false)
                dispatch(login(response.data.user))
            }
