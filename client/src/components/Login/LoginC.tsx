...
const SignUp = ()=>{
    ...
    const HandleSignUp =async(data:FieldValues)=>{
        try{
            setloader(true);
            let response = await axios.post("http://13.211.135.249:8000/api/v1/user",data,{withCredentials:true});
            if(response.data){
...
export const Login = ()=>{
    ...
    const handleLogin=async(data:FieldValues):Promise<void>=>{
        try{
            setloader(true)
            let response = await axios.post('http://13.211.135.249:8000/api/v1/user/login',data,{withCredentials:true})
            if(response.data){
                dispatch(login(response.data.user))
                setloader(false)
                navigate('/');
